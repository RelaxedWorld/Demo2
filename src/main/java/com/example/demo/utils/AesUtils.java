...
public class AesUtils {
    ...
    private static final String CHRASET_UTF8 = "UTF-8";
    ...
    public static byte[] encrypt(String content, String password) {
        try {
            ...
            byte[] byteContent = content.getBytes(CHRASET_UTF8);
    ...
    private static SecretKeySpec getSecretKey(final String password) {
        try {
            KeyGenerator kgen = KeyGenerator.getInstance(KEY_ALGORITHM);//返回生成指定算法密钥生成器的 KeyGenerator 对象
            kgen.init(128, new SecureRandom(password.getBytes()));//AES 要求密钥长度为 128
            SecretKey secretKey = kgen.generateKey();// 根据用户密码，生成一个密钥
            byte[] enCodeFormat = secretKey.getEncoded();// 返回基本编码格式的密钥
            return new SecretKeySpec(enCodeFormat, KEY_ALGORITHM);// 转换为AES专用密钥
